...
void *nms_threadFunc(void *param)
{
    threadParam_t *p = (threadParam_t*)param;
    int t_id = p->t_id;
    int rows = p->rows;
    int cols = p->cols;
    int thresh_low = p->thresh_low;
    float ** nms_matrix = p->nms_matrix;
    Mat * dx = p->dx;
    Mat * dy = p->dx;
    Mat * magnitude = p->magnitude;

    float grad_x, grad_y, grad, grad1, grad2, grad3, grad4, grad_temp1, grad_temp2, weight;
    int direction;
    for(int i = t_id; i+NUM_THREADS<rows; i+=NUM_THREADS){
        for (int j = 1; j < cols - 1; j++) {
            if (magnitude->at<float>(i, j) > thresh_low) {
...
{
public:
    int kernel, thresh_low, thresh_high, max_edge = 1024;
    float sigma;
    bool interactive;
    int row,col;
    ...
    Mat nms_pthread(Mat &dx, Mat &dy, Mat &magnitude);
...
void my_Canny::gradient(const Mat& image, Mat& dx, Mat& dy, Mat& magnitude) {
...
//非极大值抑制
Mat my_Canny::nms_pthread(Mat& dx, Mat& dy, Mat& magnitude) {
//    float grad_x, grad_y, grad, grad1, grad2, grad3, grad4, grad_temp1, grad_temp2, weight;
//    int direction;
//    Mat nms = magnitude.clone();
    float * nms_vec = new float [this->row * this->col];
    float ** nms_matrix = new float *[this->row];
    for(int i=0;i<row; i++){
        nms_matrix[i] = nms_vec + i* col;
    }
    copy_to_matrix(nms_matrix,magnitude);
    int boundary = this->kernel;

    for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
            if (i < boundary || j < boundary || i >= row - boundary - 1 || j >= col - boundary - 1) {
                nms_matrix[i][j] = 0;
            }
        }
    }
    ...
    for (int t_id = 0; t_id < NUM_THREADS; t_id++) {
        ...
        param[t_id].rows = this->row;
        param[t_id].cols = this->col;
        param[t_id].thresh_low = this->thresh_low;
        param[t_id].dx = &dx;
        param[t_id].dy = &dy;
        param[t_id].magnitude = &magnitude;
        param[t_id].nms_matrix = nms_matrix;
        pthread_create(&handles[t_id], NULL, nms_threadFunc, (void*) (param + t_id));
    }
    ...
    Mat temp = Mat(this->row, this->col,CV_32FC1,nms_vec).clone();
    delete[] nms_vec;
    delete[] nms_matrix;
    return temp;
...
//    src = canny.smooth(src);
    ...
    Mat nms = canny.nms_pthread(dx, dy, magnitude);
